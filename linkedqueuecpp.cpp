...
LinkedQueue* createLinkedQueue() {
	...
	if (pReturn != NULL) memset(pReturn, 0, sizeof(LinkedQueue));
	else printf("오류, 메모리 할당, createLinkedQueue()\n");
...
int enqueueLQ(LinkedQueue* pQueue, QueueNode element) {
	...
	if (pQueue != NULL) {
		...
		else printf("오류, 메모리 할당, enqueueLQ()\n");
	}
	return ret;
}
...
QueueNode* dequeueLQ(LinkedQueue* pQueue) {
	...
	if (pQueue != NULL) {
		if (isLinkedQueueEmpty(pQueue) == FALSE) {
			...
			pReturn->pLink = NULL;
			pQueue->currentElementCount--;
			if (isLinkedQueueEmpty(pQueue) == TRUE) pQueue->pRearNode = NULL;
		}
	}
	return pReturn;
}
