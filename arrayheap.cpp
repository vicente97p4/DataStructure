...
ArrayMaxHeap* createArrayMaxHeap(int maxElementCount) {
	...
	if (maxElementCount > 0) {
		...
		else printf("오류, 메모리 할당, createArrayList()\n");
	}
	else printf("최대 원소 개수는 0보다 커야 합니다.\n");
	return pReturn;
}
	...
	if (pHeap != NULL && pHeap->currentElementCount > 0) {
		...
		*pReturn = pHeap->pElement[1];
		...
		while (child <= pHeap->currentElementCount) {
			if ((child < pHeap->currentElementCount) && pHeap->pElement[child].key < pHeap->pElement[child + 1].key) child++;
			if (pTemp->key >= pHeap->pElement[child].key) break;
